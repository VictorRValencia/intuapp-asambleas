...
const ManageAssemblyPage = () => {
  ...
  const [assemblyStatus, setAssemblyStatus] = useState("create");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (resAss.success) {
        ...
        setAssemblyStatus(data.status || "create");
        // Parse time
        let hour = "08",
          minute = "00",
          ampm = "AM";
  ...
  const handleSubmit = async () => {
    ...
    // Fetch latest assembly data to ensure we have current status
    const currentAssRes = await getAssemblyById(assemblyId);
    if (!currentAssRes.success)
      return toast.error("Error validando estado de la asamblea");
    const currentStatus = currentAssRes.data.status || "create";
    const currentFullHour = currentAssRes.data.hour || "";
    const currentFullDate = currentAssRes.data.date || "";

    const newFullHour = `${formData.hour}:${formData.minute} ${formData.ampm}`;
    const newFullDate = formData.date;

    const timeOrDateChanged =
      newFullHour !== currentFullHour || newFullDate !== currentFullDate;
    ...
    if (timeOrDateChanged) {
      if (
        ...
        currentStatus === "registries_finalized"
      ) {
        return toast.error(
          "Para cambiar la fecha u hora debe finalizar la asamblea primero.",
        );
      }
      if (currentStatus === "finished") {
        finalStatus = "create"; // Reset to created as requested

        // --- Full Reset Logic ---
        try {
          // 1. Reset Registries
          if (currentAssRes.data.entityId) {
            const entRes = await getEntityById(currentAssRes.data.entityId);
            if (entRes.success && entRes.data.assemblyRegistriesListId) {
              await resetAssemblyRegistries(
                entRes.data.assemblyRegistriesListId,
              );
            }
          }
          ...
          // 2. Reset Questions
          if (
            currentAssRes.data.questions &&
            currentAssRes.data.questions.length > 0
          ) {
            await resetAllQuestionsAnswers(currentAssRes.data.questions);
          }

          // 3. Delete active users
          await deleteAllAssemblyUsers(assemblyId);

          // 4. Clean up powers storage
          try {
          ...
          console.error("Error during full reset:", resetErr);
          toast.warn("Algunos datos no pudieron resetearse completamente.");
        }
      }
    }
    ...
    if (res.success) {
      ...
      if (finalStatus === "create" && currentStatus === "finished") {
        toast.info("El estado de la asamblea se ha reiniciado a 'Por iniciar'");
      }
    ...
    <div className="flex flex-col w-full min-h-screen bg-[#F8F9FB]">
      ...
      <div className="flex-1 p-8 w-full max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          <Link href="/operario" className="hover:text-gray-600">
            <Home size={14} />
          </Link>
          <ChevronRight size={14} />
          <Link
            href={`/operario/${entityId}`}
        ...
        <div className="flex justify-end gap-4">
          <button
            variant="secondary"
            onClick={() => router.push(`/operario/${entityId}`)}
          ...
          <Button
            ...
            disabled={submitting}
            variant="primary"
            className="px-10 py-4 bg-[#0E3C42] hover:bg-[#08282d] gap-2 justify-center !py-2 !text-[#000000] font-bold"
          >
            {submitting && (
